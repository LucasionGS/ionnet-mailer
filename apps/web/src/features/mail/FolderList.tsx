...
function FolderItem({ folder, active, depth, icon, onDrop }: { folder: Folder; active: boolean; depth: number; icon: React.ReactNode; onDrop?: (path: string) => void }) {
  ...
  return (
    <Link
      ...
      className={cn(
        "focus-ring group flex h-8 items-center gap-2 rounded-md pr-2 text-sm transition-colors",
        active ? "bg-selected font-medium text-accent" : "text-fg-muted hover:bg-surface-2 hover:text-fg",
    ...
    >
      ...
      {folder.unread > 0 && (
        <span className={cn("rounded-full px-1.5 text-[11px] font-semibold tabular-nums", active ? "text-accent" : "text-fg-muted")}>{folder.unread}</span>
      )}
    </Link>
  );
}
    ...
    <div className="flex h-full flex-col">
      <div className="scroll-thin flex-1 overflow-y-auto px-2 py-2">
        ...
        <div className="mt-4 mb-1 flex items-center justify-between px-2">
          <span className="text-[11px] font-semibold tracking-wide text-fg-faint uppercase">Folders</span>
          <IconButton size="sm" label="New folder" onClick={() => setCreating(true)}>
            <Plus size={14} />
          </IconButton>
        </div>
        <div className="flex flex-col gap-0.5">
          {tree.length === 0 && <div className="px-2 py-1 text-xs text-fg-faint">No custom folders</div>}
          {tree.map((n) => (
